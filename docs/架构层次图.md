...
按象限权重算格子面积，每格用 `taskCard()` 铺卡片；数据读自 `S.tasksOfQuad(q)`（`store.js:222`）。
格子之间拖卡片走 `ui.js:113` 的 `pointerDrag()`，鼠标和触摸共用一套 pointer 事件，松手时算出
目标格 + 落点下标，交给 `S.moveTaskRelative()`。

**关键设计**：重渲染**不整屏重建**。每个格子 `subscribe` 自己的 `_refresh`（`quadrant.js:365`），
顶栏统计另有 `shell.js:1393` 的订阅，所以勾一个任务只刷那一格 + 那一行数字。详情抽屉
