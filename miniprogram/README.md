...
│  ├─ plugins/    # 三端统一插件中心：13 个内置插件清单/启停/平台能力
...
| 插件系统 | 同步 13 个插件清单与启停状态；其中 9 个提供小程序原生适配 | 小程序不能执行桌面 DOM 插件，也没有 Tauri Rust 网络桥，因此按插件能力做原生适配/降级 |
...
设置 → **插件中心** 会显示与 Windows / Android 完全相同的 13 个内置插件及版本。启停状态保存在同一个 `plugins` 字段里，备份 JSON 跨端导入后会继续生效。 tabBar 与插件中心图标由桌面端同一份 Font Awesome Free SVG sprite 生成，避免三端图标体系分叉。
...
- **插件使用说明**：分组卡片实时读清单显示版本与平台能力；外链降级为复制仓库地址。
- **微信提醒推送**：PushPlus / Server酱 配置与测试消息；推送检查只在插件页打开期间每分钟一次（桌面端常驻）。
- **竞赛消息雷达**：抓取摩课云公告，关键词/类型/月份过滤 + 已读标记；详情链接复制到浏览器打开。
- **学习通**：账号密码（DES 加密）/ Cookie 双登录，收件箱全文、待办作业（含「正在批改」探测）、课程学年推断、分享码查询，均可一键转提醒；存储键与桌面端同名，备份互通。核心纯逻辑在 `core/chaoxingCore.js`，DES 与 pyDes 实测向量对照。
- **轮换值日**：把「按成员顺序轮换」的公共事务集中管理，**一个插件里可放多套互相独立的轮换**（宿舍值日 / 公区卫生…），各有自己的成员、周期、起始日与提醒时刻。每套按「起始日 + 周期切段、一段一人」，成员增删改序、误删可恢复、某轮可临时换人；可一键把本轮加入今日任务。轮换数学在 `core/pluginRuntime.js` 的 `dd*` 一组纯函数里，与桌面端同一套规则；存储键与桌面端逐字一致，备份跨端互通。小程序不能常驻后台，**到点提醒只在打开插件页时补一次**（每轮第一天 + 已过设定时刻 + 当天没提醒过），需要系统通知请用桌面 / Android 端。

警大门户通知、课程表、网页收集、学校通知网站仍保留在统一清单中，但小程序端暂标记为不可用：网页收集与学校通知网站需请求**任意**域名，小程序合法域名白名单机制不允许；警大门户的验证码 OCR 与课程表的 DOM 周视图尚未找到小程序等价实现（后者列入下批 WXML 移植）。

**联网插件域名白名单**：正式发布前需在小程序后台「开发设置 → 服务器域名」登记
`notice.chaoxing.com`、`passport2.chaoxing.com`、`mooc2-ans.chaoxing.com`、`sharewh3.xuexi365.com`、
`www.gxxsjs.com`、`www.pushplus.plus`、`sctapi.ftqq.com`；开发工具与体验版可先勾选「不校验合法域名」。

**学习通登录注意事项**：个别机型 `wx.request` 拿不到 `Set-Cookie` 响应头，账号密码登录后会话无法保持，此时改用「Cookie 登录」（从桌面端插件复制 Cookie 粘贴进来即可，两端存储键同名）。
...
```bash
node tools/test-miniprogram-core.js        # store + timeParser + captureFlow + 插件纯逻辑 共 150 个用例（mock wx）
