...
- **裁剪**：每源保留最近 60 条，但**未读与收藏不受条数限制** —— 否则会出现「还没看就被裁掉」，
  未读数自己往下掉。
- **预置源**：新装用户首屏就有 4 个（少数派 / 阮一峰的网络日志 / InfoQ 中文 / 橘鸦AI早报），
  订阅管理里另给 4 个推荐源一键添加。
  `npm run check:feeds` 会用**真解析器**逐个体检（状态码 / 内容类型 / 条目数 / 摘要长度 /
  封面数），改过源清单就该跑一次 —— 预置源写错的表现是「每个新装用户首屏都有一块抓取失败」，
  而 `npm test` 必须离线可跑、管不到它。实测踩过：一条 GitHub Pages 已迁站的 404 地址
  （`imjuya.github.io/juya-ai-daily/rss.xml`）差点当成可用源发出去，现址是 `daily.juya.uk/rss.xml`。
...
- `scripts/test-rss-reader.mjs` 新增「预置源形态」一节（`description` 必须先于
  `content:encoded` 命中、摘要上限正好 220、`guid` 与 `link` 不同时身份认 `guid`、
  无图条目封面为空），**5 条变异全部被抓住**。首轮有 1 条漏过：夹具里的 `description`
  只有 60 字，短于 220 ⇒ `snippet.length <= 220` 恒真，把上限放开到 5000 也照样绿
  ⇒ 夹具加长到与真实源同量级（约 360 字）、判据改成精确值 220 才捕获。
- 探针里的夹具规模（源数 / 条目总数）原先硬编码在 12 处（`3` 与 `33`），加第 4 个预置源时
  三档断言全红。已收成生成时算出的 `TOTAL` / `NSRC` 两个常量，以后加删源只动夹具。
