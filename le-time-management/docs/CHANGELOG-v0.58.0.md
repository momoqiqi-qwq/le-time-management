...
## 修复：一段重复的注释吃掉了 `body` 基础规则（**v0.57.0 起就已存在**）

### 怎么发现的

本版构建时 vite 打了一行很容易刷过去的告警：

```
▲ [WARNING] Unexpected "*" [css-syntax-error]   <stdin>:230:21
      230 │    这是本功能的**已知边界，不是缺陷**：缩放只…
```

追下去发现是 `src/styles.css` 里那段讲「zoom 与断点」的大注释**结尾被整段复制了一遍**：
第一个注释结束符提前把注释关掉，第二份正文裸露成了样式表正文，末尾还留了个孤儿结束符。

### 为什么后果严重

浏览器把这段裸文本当成**选择器**往下吞，直到撞上 `body {` 的 `{` ——
选择器因此非法，**整条 `body` 规则被丢弃**。无头 Chrome 实测（同一份构建产物）：

| | font-family | overflow | color |
|---|---|---|---|
| 修复前 | `"Noto Sans SC"`（兜底字体） | **visible** | 默认黑 `rgb(0,0,0)` |
| 修复后 | `"Segoe UI", "Microsoft YaHei", …` | `hidden` | `--ink` `rgb(34,48,58)` |

⇒ 字体族、`overflow:hidden`、正文颜色**全部静默失效**，没有任何报错。
`git show beed8d7:le-time-management/src/styles.css` 里两处重复都在 ⇒
**v0.57.0 及更早已带着这个缺陷发布**，不是本版引入。

### 改了什么

| 改动 | 文件 | 说明 |
|---|---|---|
| 删掉重复的注释正文与孤儿结束符 | `src/styles.css` | 只删重复段落，第一份注释完整保留 |
| 新增回归守卫 | `scripts/test-css-integrity.mjs` | 见下 |

### 回归守卫（新脚本，全量 52 个之一）

1. **结构扫描**（自带 tokenizer，零依赖）：扫 `src/` `public/` `miniprogram/` 下全部
   `.css` / `.wxss`，查**孤儿注释结束符**、未闭合注释、花括号配平。跳过字符串字面量
   （`content: "*​/"` 之类不能误报）。根因就是孤儿结束符，所以这条确定能抓住本 bug。
2. **esbuild 解析告警必须为 0** —— 覆盖其它畸形 CSS。
   ⚠️ 刻意**不**用「产物里还有没有 `body{…}`」当判据：esbuild 的恢复能力比浏览器强，
   它在告警的同时照常输出 `body{…}`，只有**告警数**才有分辨力。
3. **正面锚点**：产物里必须存在一条同时含 `overflow:hidden` 与 `font-family` 的 `body{…}`
   （排除 `html,body{height:100%}` 那条），钉住「该在的确实在」。

**变异验证**（3/3 被拦下）：① 把孤儿结束符塞回去 ⇒ 结构扫描报错；
② 删掉 `body` 的 `overflow:hidden` ⇒ 正面锚点报错；③ 删一个 `}` ⇒ 配平报错。
还原后 sha 与变异前逐位一致。

### 影响端

三端共用同一份 `styles.css`（小程序有自己的 WXSS，未受影响）。无数据迁移。
