...
## 修：APK 点开任意插件时底栏「闪/跳一帧」

### 现象（用户真机反馈 + 追问确认）

Android 端点开**任意插件**，底部导航栏会闪/跳一帧；四象限、时间块、收件箱等核心页没有此现象。

### 排查（无头 Chrome + 390×844 Android UA 探针，`output/preview/phone-demos/__plugin-tap-probe.html`）

16ms 采样 325 拍：`.rail` 矩形全程零位移、文档不可滚动（`scrollHeight = clientHeight = 844`）、
强制滚 html 不带动底栏、`switchTo` 链路无任何移动底栏的类切换 —— **布局层排除**。
键盘起落、双击缩放两个真机嫌疑也由用户确认排除（形态是「底栏自己闪」，不跟键盘、不整页缩放）。

### 根因

插件视图与核心页在 `switchTo()` 里只有一处分支差异：插件会走
**`observePluginMotion()`**（`src/motion.js`，宿主给插件补的轻量入场反馈）——
插件挂载和数据到达时对新节点跑 WAAPI 透明度动画（最多 12 个根节点）+ 大段 DOM 替换，
触发整帧重光栅化。固定定位的底栏（`.rail`，`position:fixed` + 阴影）若与内容同层，
就会被拖着重绘这一帧 —— 真机 WebView 上表现为「闪一下」。核心页同步一次成型、
不走 `observePluginMotion`，所以不弹。

### 改法（`src/styles.css`，≤900px 底栏规则一处）

`.rail` 加 `transform: translateZ(0)`，把固定底栏提升为**常驻合成层**：
内容层再怎么翻腾（插件挂载/动画/重绘），底栏的层不动、不被拖着重绘。
只加在窄屏媒体块内 —— 桌面端 `.rail` 是普通侧栏，无此问题。

### 影响端

- Android：✅ 修复（需重出 APK 生效）
- Windows：≤900px 规则不命中桌面窗口，无行为影响
- 小程序：不涉及（无此底栏）

### 验证

- 复跑切换探针：底栏矩形与修复前逐字节一致（780.43 / 844.19 / 63.76），两阶段 `MOVED: false`，零布局回归。
- 四道 `--check` 全 ✓（v0.51.0）；`run-tests.mjs` **41/41 全过**。
- 待真机复核：装新 APK 后点各插件，底栏应不再闪。
...
托盘右键菜单的「退出」原先是「广播 `app-quit` → 无条件 `sleep(800ms)` → `app.exit(0)`」。
`app.exit(0)` 会**直接掐掉进程**，而前端收到事件后要跑
`saveNow()` → invoke `save_data` → Rust 写 `data.json`，这条链路要穿一次 IPC。

800ms 是拍脑袋定的 —— 慢机器上没跑完，**用户最后的改动就静默丢了**，
而且丢得毫无征兆（不报错、不提示）。
...
| 位置 | 改动 |
|---|---|
| `src-tauri/src/lib.rs` | 新增 `QuitGate { acked: Mutex<bool>, cv: Condvar }`，用 `wait_timeout_while` 等前端回执，**最长 2s**；超时照常退出 |
| 同上 | 新增 `quit_ack` 命令（已注册进 `generate_handler!`），前端存完盘后调用它唤醒退出线程 |
| `src/main.js` | `listen("app-quit")` 里改成 `saveNow().then(ack, ack)` —— **成功、失败两个槽位都发回执** |
| `src/api.js` | 新增 `api.quitAck()`，按 `isTauri` 降级（浏览器里是空操作） |

三个设计决定：

1. **回执必须成功失败都发。** 只挂成功槽（`.then(ack)`）的话，写盘报错时 Rust 收不到回执，
   照样白等满 2s —— 那就退化成「慢版的原实现」。
2. **必须带超时。** 用无超时的 `wait` 的话，前端一旦崩了或压根没加载完，退出线程会**永久挂住** ——
   「点了退出退不掉」比丢数据更难受。超时兜底的选择是明确的：宁可丢一次写盘，也不能让用户退不掉。
3. **固定 `sleep` 要删掉，不是留着并存。** 两条路并存时实际生效的是先到的那条，
   慢机器上 `sleep` 仍会抢跑于存盘完成，握手机制被整个架空。

**实际效果**：正常情况下前端存完盘立刻回执，退出比原来**更快**（不用干等 800ms）；
只有异常时才退到 2s 超时。

### 验证

- `scripts/test-desktop-tray.mjs` 新增第八、九、十节（共 19 条新断言）：广播必须在等待之前、
  等待必须在 `app.exit` 之前、等待必须带超时谓词、`quit_ack` 必须注册进 `generate_handler!`、
  前端回执必须双槽位、`api.quitAck` 必须按 `isTauri` 降级，
  以及**反面断言**（固定 `sleep` 必须消失、不能写成只挂成功槽的 `.then(ack)`）。
- 配套变异测试：**10 条变异逐条改坏实现，全部被拦下**
  （含「sleep 与回执并存」「先 exit 再等回执」「回执等待去掉超时」「漏注册命令」等）。
