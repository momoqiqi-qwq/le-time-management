...
(function () {
  ...
  function render(el) {
    ...
    msBtn.addEventListener("click", () => { msPanel.hidden = !msPanel.hidden; });
    msBoxes.forEach((b) => b.addEventListener("change", async () => { readFields(); await save(); paintScope(); }));
    document.addEventListener("click", (e) => { if (!e.target.closest("[data-ms]")) msPanel.hidden = true; });
