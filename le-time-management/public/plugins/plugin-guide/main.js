(function(){
  ...
  const docs={
    "shiguang-schedule":["打开课程表，先设置学期与开学日期","可手动添加，或走“教务导入”：选学校登录教务，或改用文件 / 表格导入","确认预览后选择合并或替换"],
    ...
    "dorm-duty":["一个插件里可放多套轮换（宿舍值日 / 公区卫生…），各有自己的成员、周期与提醒时刻，互不影响","选中一套轮换后按顺序添加成员，第一个人先当班；设好起始日期与轮换周期（每天 / 每周 / 自定义 N 天）","支持多人值日：在「轮换规则 → 每轮人数」里选 2～4 人（或自定义 N），每轮就按名单顺序 N 人一起当班","需要时给某一轮临时换人（可勾选多人）；到点会提醒当班的人，也可一键加入今日任务","标签可以右键（小程序端点标签后面的「⋯」）：除了切组、改名、删除，还能把整套轮换连成员带规则复制一份再改名，或从别的轮换按名字导入成员，不用重打一遍名字"],
